...
paths = [
    ...
    "HTML Charts",
    "HTML Charts/Year",
    "HTML Charts/Year and Month",
    "HTML Charts/Year and Month and ID",
    "HTML Charts/Year and Quarter",
    "HTML Charts/Year and Quarter and ID",
]
...
try:
    ...
    data["eventpartofday"] = data["eventhour"].apply(part_of_day)
    ...
    updated_data_.to_csv(
        "./csv files/data.csv", index=False, header=True, encoding="utf-8"
    )
    data.to_csv("./csv files/main_data.csv", index=False, header=True, encoding="utf-8")
...
def authenticate_user(user_db, username: str, password: str):
    if user := get_user(user_db, username):
...
async def get_data_per_driver(DriverID: int):
    if DriverID not in updated_data_["DriverID"].values:
...
async def get_data_per_month(month: str):
    if month in updated_data_["eventmonth"].values:
        ...
        return {"message": "Success", "status": 200, "data": result}
    return {
        "message": "Not enough record for this month. Please select another month",
        "status": 401,
    }
...
async def get_data_per_month_per_driver(month: str, DriverID: int):
    if month in updated_data_["eventmonth"].values:
        data = updated_data_[
            (updated_data_["eventmonth"] == month)
...
async def get_driver_score_per_year(year: int):
    ...
    if day.shape[0] < 600:
        return {
            "message": "Not Enough Records, Please Select Another Month",
            "status": 401,
        }
    else:
        day = data[data["eventyear"] == year]
        ...
        eventsPerDriver = (
            day.groupby("DriverID", as_index=True)
            .agg({"event": "count"})
        ...
        fig = px.histogram(eventsPerDriver.reset_index(), x="Number of event")
        fig.update_layout(
        ...
        fig = px.bar(
            dfReasonHist, x="Number of event", y="event", orientation="h", color="event"
        )
        fig.update_layout(
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year/Bar Plot of Event Distribution.html")
        ...
        behavioral_events = [
            ...
            if event
            in ["hardAcceleration", "hardBraking", "hardCornering", "overspeed"]
        ]
        ...
        def prepData(day, minRecordsPerSubscriber=0):
            ...
            eventCountPerdriver = df4.groupby("DriverID")["DriverID"].agg("count")
        ...
        df6 = prepData(day)
        relevantEventsPerSubscriber = (
            df6.groupby("DriverID")
            .agg({"event": "count"})
        ...
        dfReasonHist = (
            df6.groupby("event")[["event"]]
            .agg("count")
            .add_prefix("Number of ")
            .reset_index()
            .sort_values("Number of event", ascending=False)
        )
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year/Distribution of Behavioral Events.html")

        # Hard Acceleration Visual
        hard_acceleration = day[day["event"] == "hardAcceleration"]
        ha = (
            hard_acceleration[["DriverID", "event"]]
            .groupby("DriverID")
            .agg("count")
            .add_prefix("Number of ")
        )
        ...
        fig = px.bar(
            distancePerdriver.sort_values(by="Distance", ascending=True),
            x="DriverID",
            y="Distance",
            color="Distance",
        )
        ...
        def create_feature_set(df6, distancePerdriver):
            dfEventAggByDriver = (
                df6.groupby(["DriverID", "event"])[["event"]]
                .agg("count")
                .add_prefix("Number of ")
                .reset_index()
            )
            ...
            dfEventMatrix = (
                dfEventAggByDriver.pivot(
                    index="DriverID", columns=["event"], values="Number of event"
                )
                .add_prefix("F_")
                .fillna(0)
                .reset_index()
        ...
        features_log = features.replace([np.inf, -np.inf], np.nan, inplace=False)
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year/scatter_matrix_log_transformation.html")
        ...
        transFeatures = features.apply(
            lambda x: (transform_to_normal(x, min_max_transform=True))
        )
        transFeatures = transFeatures.replace([np.inf, -np.inf], np.nan, inplace=False)
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year/scatter_matrix_cleaned_features.html")
        ...
        transFeaturesScaled = (
            transFeatures - transFeatures.mean()
        ) / transFeatures.std()
        ...
        def fit_distribution_params(series):
            ...
            xPositive = xPositive.replace([np.inf, -np.inf], np.nan, inplace=False)
            ...
            if len(xPositive) > 0:
                ...
                if verbose:
                    probs_df = pd.DataFrame(
                        {"Event value": x.values.tolist(), "Event probability": probs},
                        index=True,
                    )
        ...
        perFeatureScores = normalizedFeatures.apply(
            calculate_score_for_series, args=(fittedParams, False), axis=0
        ).add_suffix("_CDF")
        ...
        def calculate_joint_score(perFeatureScores):
            ...
            featureCols = [
                col
                for col in driverScores
                if col.startswith("Hard") | col.startswith("Over")
            ]
            driverScores["score"] = 1 - (
                (driverScores[featureCols].sum(axis=1) / len(featureCols))
            )
            driverScores = driverScores.sort_values("score")
            driverScores["rank"] = (
                len(driverScores["score"]) - rankdata(driverScores["score"]) + 1
            )
            return driverScores
        ...
        driverScores["Safety Class"] = driverScores["Score"].apply(condition)

        driverScores1 = driverScores[["Driver ID", "Score", "Position", "Safety Class"]]
        driverScores1 = driverScores1.sort_values(by="Score", ascending=False)
...
async def get_driver_score_per_year_per_month(year: int, month: str):
    ...
    if day.shape[0] < 600:
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month/Histogram of Event Counts.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month/Distance Traveled per Driver.html")
        # Feature Engineering
        # Transform the events data frame to a features data frame (column for each type of relevant event)
        ...
        # fig.show()  # type: ignore
        fig.write_html("./HTML Charts/Year and Month/scatter_matrix_features.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month/scatter_matrix_features_log.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month/scatter_matrix_box_cox.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month/Bar Chart of Driver Scores.html")
        return {"message": "Success", "status": 200, "data": result}


@app.get("/getAnnualScorePerMonthPerDriver/{year}/{month}/{id}")
async def get_driver_score_per_driver_per_year_per_month(
    year: int, month: str, id: int
):
    data = updated_data_
    day = data[(data["eventyear"] == year) & (data["eventmonth"] == month)]
    if day.shape[0] < 600:
        return {
            "message": "Not Enough Records, Please Select Another Quarter",
            "status": 401,
        }
    else:
        day = data[(data["eventyear"] == year) & (data["eventmonth"] == month)]
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month and ID/Event Distribution.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month and ID/scatter_matrix.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Month and ID/correlation_heatmap.html")
...
async def get_driver_score_per_year_per_quarter(year: int, quarter: int):
    ...
    if day.shape[0] < 600:
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter/Histogram of Event.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter/Histogram of Event Counts.html")
        ...
        # fig.show()  # type: ignore
        fig.write_html("./HTML Charts/Year and Quarter/scatter_matrix_features.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter/scatter_matrix_box_cox.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter/Correlation Heatmap.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter/Bar Chart of Driver Scores.html")
        return {"message": "Success", "status": 200, "data": result}


@app.get("/getAnnualScorePerQuarterPerDriver/{year}/{quarter}/{id}")
async def get_driver_score_per_driver_per_year_per_quarter(
    year: int, quarter: int, id: int
):
    data = updated_data_
    ...
    if day.shape[0] < 600:
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter and ID/Event Distribution.html")
        ...
        # fig.show()  # type: ignore
        fig.write_html("./HTML Charts/Year and Quarter and ID/scatter_matrix_features")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter and ID/scatter_matrix.html")
        ...
        # fig.show()
        fig.write_html("./HTML Charts/Year and Quarter and ID/correlation_heatmap.html")
